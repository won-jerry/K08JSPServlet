...
public class BoardDAO extends JDBConnect {
	...
	//사용자가 입력한 내용을 board 테이블에 입력(insert) 처리한다.
	...
	public BoardDTO selectView(String num) {
		...
		BoardDTO dto = new BoardDTO();
		//join을 이용해서 member 테이블의 name 컬럼까지 가져온다.
		...
		try {
			...
			/*
			일련번호는 중복되지 않으므로 if문으로 처리한다. 게시판 목록처럼
			여러개의 레코드를 가져온다면 while문을 사용하면 된다.
			 */
			if (rs.next()) {
				//DTO에 레코드의 내용을 추가한다.
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map) {
		...
		인출할 수 있는 쿼리문을 작성한다. (자세한 내용은 JDBC 실습을.sql 참조)
		 */
		...
		try {
			...
			인파라미터 설정 : JSP에서 해당 페이지에 출력할 게시물의 구간을
				계산한 후 Map 컬렉션에 저장하고 DAO로 전달하면 해당 값으로
				쿼리문을 완성한다.
			 */
